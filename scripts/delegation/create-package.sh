...
# Build the package
{
  ...
  echo ""
  echo "1. Create branch: \`feature/${ISSUE_NUM}-brief-description\`"
  echo "2. Use conventional commit format: \`fix:\` or \`feat:\`"
  echo "3. Follow acceptance criteria from spec above"
  echo "4. Run these before pushing:"
  echo "   - \`pnpm typecheck\`"
  echo "   - \`pnpm lint\`"
  echo "   - \`pnpm test\`"
  if [ -n "$SPEC_FILE" ]; then
    echo "5. Reference Spec ID: SPEC-${ISSUE_NUM}"
  fi
  echo ""
  echo "---"
  echo ""
