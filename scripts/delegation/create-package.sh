...
# Build the package
{
  ...
  echo ""
  echo "### Project Setup (IMPORTANT - Run First!)"
  echo ""
  echo "Before starting implementation, install dependencies:"
  echo ""
  echo "\`\`\`bash"
  echo "# Install all dependencies (required for lint, typecheck, build, test)"
  echo "pnpm install"
  echo ""
  ...
  echo "\`\`\`"
  echo ""
  echo "⚠️ **If \`pnpm install\` fails**, you may need to install pnpm first:"
  echo "- \`npm install -g pnpm\` (if you have npm)"
  echo "- \`curl -fsSL https://get.pnpm.io/install.sh | sh -\` (standalone installer)"
  echo ""
  echo "### Git Setup"
  echo ""
