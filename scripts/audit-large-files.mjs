...
const BAD_LIMIT =
  Number.parseInt(process.env.AUDIT_LARGE_LIMIT_BYTES ?? "", 10) ||
  5 * 1024 * 1024; // 5MB default; override via env

const ALLOW_PREFIXES = (process.env.AUDIT_LARGE_ALLOW_PREFIXES ?? "public/,docs/reports/,apps/web/public/,.turbo/,bin/")
  .split(",")
  .map(s => s.trim())
  .filter(Boolean);
...
console.log(`Top 50 largest blobs (limit: ${BAD_LIMIT} bytes, allowed: ${ALLOW_PREFIXES.join(', ')}):\n`);
