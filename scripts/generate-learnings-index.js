...
function generateIndex(lessons, useHeatSorting = false, totalUsageEvents = 0) {
  ...
  if (lessons.length === 0) {
    ...
    lessons.forEach((lesson, index) => {
      ...
      // Show usage count when heat ranking is active (capped at 3× display)
      if (useHeatSorting && lesson.usedBy > 0) {
        const displayUsage = Math.min(3, lesson.usedBy);
        const suffix = lesson.usedBy > 3 ? '+' : '';
        content += ` (${displayUsage}×${suffix})`;
      }
      content += '\n';
    });
    content += '\n';
