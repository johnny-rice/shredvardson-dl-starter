...
const TEST_DIRS = ['.', 'tests', 'e2e', '__tests__', 'apps/web/tests', 'apps/web/e2e', 'apps/web/__tests__'];
...
function checkTestFile(filePath) {
  try {
    ...
    if (content.includes('beforeEach') && 
        /beforeEach[\s\S]*?(?:localStorage|sessionStorage|clearAllMocks|resetModules|\.clear\(\)|\.reset\(\))/m.test(content)) {
      violatingFiles.push({
        file: filePath,
